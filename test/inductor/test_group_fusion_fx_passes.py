    ...
    def test_group_layer_norm(self):
        class TestModule(torch.nn.Module):
            def __init__(self):
                ...
                self.l2_ws = [torch.nn.Parameter(torch.randn(10)) for _ in range(5)]
                self.l2_bs = [torch.nn.Parameter(torch.randn(10)) for _ in range(5)]

            def forward(self, x):
                post_l1 = []
                for l1_out, l1_w, l1_b in zip(
                    torch.split(x, 10, dim=1), self.l1_ws, self.l1_bs
                ):
                    post_l1.append(
                ...
                l1_out = torch.cat(post_l1, dim=1)

                post_l2 = []
                for l2_out, l2_w, l2_b in zip(
                    torch.split(l1_out, 10, dim=1), self.l2_ws, self.l2_bs
                ):
                    post_l2.append(
                        torch.nn.functional.layer_norm(
                            l2_out, (10,), weight=l2_w, bias=l2_b
                        )
                    )

                return torch.cat(post_l2, dim=1)
        ...
        self.assertEqual(
            counters["inductor"]["ln_removed"],
            10,
        )
        self.assertEqual(
            counters["inductor"]["ln_added"],
            2,
        )
        counters.clear()
