...
class FxirTestCase(InductorTestCase):
    ...
    def test_fallback_tuple_constant_arg(self):
        ...
        # The second argument should be the permutation tuple (0, 2, 1)
        # Check that it's a tuple and not flattened
        perm_arg = permute_node.args[1]
        self.assertIsInstance(perm_arg, tuple, "Permutation argument should be a tuple")
        self.assertEqual(len(perm_arg), 3)
        self.assertEqual(tuple(perm_arg), (0, 2, 1))

    def test_clone_memory_format(self):
        """
        Test torch.clone with memory_format keyword argument.
        This tests an operation with keyword-only args after *.
        """

        def foo(x):
            # Clone with explicit memory_format
            return x.clone(memory_format=torch.contiguous_format) + 1

        args = [torch.randn(4, 8, device=self.device)]
        (gm,) = self._compile_and_check(foo, args, expected_num_triton_kernels=2)

        # Check for clone operation
        num_clone = self._count_ops(gm, torch.ops.aten.clone.default)
        self.assertEqual(num_clone, 1)
