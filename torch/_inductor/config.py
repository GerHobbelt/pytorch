...
import torch
from torch._dynamo.config_utils import ConfigMixin


def decide_compile_threads(config):
    """
    ...
    elif sys.platform == "win32" or config.is_fbcode():
        return 1
    else:
        return min(
            32,
...
class InductorConfig(ConfigMixin):
    ...
    developer_warnings: bool = is_nightly_or_source
...
default_config = InductorConfig()
if default_config.is_fbcode():
    default_config.global_cache_dir = "fb/cache"
    default_config.developer_warnings: bool = True
    default_config.disable_cpp_codegen: bool = True

default_config.compile_threads = decide_compile_threads(default_config)
