    ...
    if config.is_fbcode():
        if aot_mode and not use_absolute_path:
            ...
            if " " not in input:
                ...
                # Multiple inputs
                inp_name = " ".join([os.path.basename(i) for i in input.split()])
        ...
        if key not in cls.cache:
            ...
            with lock:
                ...
                if not os.path.exists(output_so):
                    ...
                    if use_absolute_path:
                        compile_file(f"{output_o} {consts_o}", output_so, cmd.split())
...
def compile_file(input_path: str, output_path: str, cmd: List[str]) -> None:
    if " " not in input_path:
        ...
        input_files = [os.path.basename(input_path) if config.is_fbcode() else input_path]
    else:
        ...
        if config.is_fbcode():
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                for i in range(len(input_paths)):
                    shutil.copy(input_paths[i], os.path.join(tmp_dir, input_files[i]))
