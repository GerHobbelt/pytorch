...
class PallasKernel(SIMDKernel):
    ...
    def _compute_store_coeffs(
        ...
        """Map each RT var to its store-side linearization coefficient.

        Returns ``{sympy.Symbol: int}`` or ``None`` on failure.
        """
    ...
    def _get_full_load_permutation(
        ...
        Maps RT vars to input/output dims via index and store coefficients.
        """
        ...
        if len(ordered) != len(in_shape):
            # Single flat RT variable — infer perm from output vs input strides.
            n = len(in_shape)
            if (
                len(ordered) == 1
                and self._safe_int(
    ...
    def _get_collapsed_load_permutation(
        ...
        """Detect permutation when the range tree has fewer vars than the buffer rank.

        Returns (collapsed_input_shape, perm) or None.
        """
        info = self._get_buffer_info(name)
        if not info:
            return None
